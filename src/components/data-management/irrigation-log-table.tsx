...
interface IrrigationLogTableProps {
...
export function IrrigationLogTable({ refreshTrigger, onLogDeleted }: IrrigationLogTableProps) {
  ...
  useEffect(() => {
    ...
    const fetchIrrigationLogs = async () => {
      try {
        ...
        const fetchedLogsPromises = querySnapshot.docs.map(async (docSnapshot) => {
          ...
          if (data.fieldId) {
            try {
              ...
              console.error("Error fetching field name for irrigation log:", docSnapshot.id, fieldError);
        ...
        console.error("Failed to load irrigation logs from Firestore:", e);
        setError("Could not load irrigation logs. Please try again.");
        toast({
  ...
  const handleDeleteLog = async (logId: string) => {
    ...
    if (window.confirm("Are you sure you want to delete this irrigation log? This action cannot be undone.")) {
      try {
        await deleteDoc(doc(db, "irrigationLogs", logId));
        toast({
          title: "Irrigation Log Deleted",
          description: "The log has been removed from Firestore.",
        });
        onLogDeleted(); 
      } catch (e) {
        console.error("Failed to delete irrigation log:", e);
  ...
  if (isLoading) {
    return <p className="text-center text-muted-foreground py-4">Loading irrigation logs...</p>;
  }
  ...
  if (logs.length === 0 && user?.farmId) {
    return (
      <Alert className="mt-4">
        <Icons.Info className="h-4 w-4" />
        <AlertTitle>No Irrigation Logs Found</AlertTitle>
        <AlertDescription>
          No irrigation logs found for this farm. Add a new log using the form.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="mt-8 border rounded-lg shadow-sm overflow-x-auto">
      <Table>
        <TableCaption>A list of your farm's recent irrigation logs.</TableCaption>
        <TableHeader>
          <TableRow>
            <TableHead className="min-w-[150px]">Field</TableHead>
            <TableHead className="min-w-[120px]">Date</TableHead>
            <TableHead className="min-w-[150px]">Water Source</TableHead>
        ...
        <TableBody>
          {logs.map((log) => (
            <TableRow key={log.id}>
              ...
              <TableCell>{format(parseISO(log.irrigationDate), "MMM dd, yyyy")}</TableCell>
              <TableCell>{log.waterSource}</TableCell>
